"""Release data for Danata.

When Danata is imported a number of steps are followed to determine
the version information.
   ...
   1) If the release is not a development release (dev=False), then version
      ...
      downloadable release of Danata since setup.py creates it during
      packaging/installation.  However, version.py might not exist if one
      is running Danata from the mercurial repository.  In the event that
...
Danata, we must make sure write_versionfile() does not overwrite the
...
# Was Danata built from a development version? If so, remember that the major
